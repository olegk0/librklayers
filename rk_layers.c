...
//    return ioctl(Ovl_priv.OvlFb[UILayer].fd, FBIOSET_COLORKEY, &color);
	...
	return ioctl(Ovl_priv.OvlFb[UILayer].fd, RK_FBIOPUT_COLOR_KEY_CFG, &colorkeys);
...
//    if((xres > Ovl_priv.cur_var.xres)||(yres > Ovl_priv.cur_var.yres)) return -1;
    	if(format != RK_FORMAT_DEFAULT)
    		Ovl_priv.OvlLay[layout].var.nonstd = (Ovl_priv.OvlLay[layout].var.nonstd & ~0xff) | ovlToHWRkFormat(format);
    	if(xres>0){
...
int OvlSetupDrw( OvlLayPg layout, int Drw_x, int Drw_y, int Drw_w, int Drw_h)
{
    ...
    if(LayValidAndNotUI(layout)){
    	Ovl_priv.OvlLay[layout].var.grayscale = (Drw_w << 8) | (Drw_h << 20);
    	Ovl_priv.OvlLay[layout].var.nonstd &= 0xff;
    	Ovl_priv.OvlLay[layout].var.nonstd |= (Drw_x<<8) + (Drw_y<<20);
...
{       
    ...
    if(LayValidAndNotUI(layout)){
    	...
    	ret = OvlLayerLinkMemPg( layout, Ovl_priv.OvlLay[layout].FbMemPgs[Ovl_priv.OvlLay[layout].FbBufUsed]);
...
{
//	OVLDBG("layout:%d en:%d", layout, enable);
    if(LayValidAndNotUI(layout)){
    	int tmp = 0;
		ioctl(FbByLay(layout)->fd, RK_FBIOSET_VSYNC_ENABLE, &tmp);
...
//---------------------------------------------------------------------
int ovlUsed( OvlLayPg layout)
{
	int used, ret;

//for extern func    if(LayValidAndNotUI(layout))
    	ret = ioctl(FbByLay(layout)->fd, RK_FBIOGET_ENABLE, &used);
    	OVLDBG("layout:%d ret:%d used:%d", layout, ret, used);
    	if(ret)
    		return ret;
    	else{
    		if(!used)
    			OvlEnable(layout, 0); // workaround
    	}
    	return used;
//    else
//    	return -ENODEV;
...
//    case NOT_SCALEL:
    	for(i=0;i < Ovl_priv.OvlsCnt;i++){
    		if(FbByLay(i)->Type == type){
    			if(!Ovl_priv.OvlLay[i].InUse && !ovlUsed(i)){
    				lay = i;
    				break;
    			}
/*    			else{
...
void OvlFreeLay( OvlLayPg layout)
{
    ...
    if(LayValid(layout)){
    	if(Ovl_priv.OvlLay[layout].InUse){
...
//    	Ovl_priv.OvlLay[i].ResChange = FALSE;
    	...
    	if(i>0){
        	Ovl_priv.OvlFb[i].Type = SCALEL;//        UIL=0,    SCALEL=1,    NotSCALEL=2, TODO HWRGALayer
